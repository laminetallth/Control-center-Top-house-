...
function elimina(id){ if(!confirm("Eliminare questo documento?")){ return; } salva(leggi().filter(d => Number(d.id) !== Number(id))); render(); }
documentForm.addEventListener("submit", e => { e.preventDefault(); const lista = leggi(); const id = documentId.value; const dati = { id: id ? Number(id) : Date.now(), titolo: testo(titolo.value), categoria: categoria.value, collegatoA: testo(collegatoA.value), linkDocumento: testo(linkDocumento.value), scadenza: scadenza.value, stato: stato.value, note: testo(note.value), aggiornatoIl: new Date().toISOString() }; const i = lista.findIndex(d => Number(d.id) === Number(id)); if(i >= 0){ lista[i] = dati; }else{ lista.push(dati); } salva(lista); resetForm(); render(); });
