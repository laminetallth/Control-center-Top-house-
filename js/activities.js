...
function modifica(id){ const a = leggi().find(x => Number(x.id) === Number(id)); if(!a){ return; } ["activityId","titolo","descrizione","priorita","scadenza","assegnatoA","note"].forEach(k => { const el = document.getElementById(k); if(el){ el.value = k === "activityId" ? a.id : (a[k] || ""); } }); document.getElementById("saveActivityBtn").innerText = "Aggiorna attività"; window.scrollTo({top:0, behavior:"smooth"}); }
